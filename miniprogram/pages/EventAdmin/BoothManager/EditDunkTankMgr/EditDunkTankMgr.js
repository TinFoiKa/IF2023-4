// pages/EventAdmin/BoothManager/EditDunkTankMgr/EditDunkTankMgr.js
Page({
    ...
    changeStatus : function(event){
        ...
        wx.cloud.callFunction({
            name : "executeSql",
            data : {
				sql : "update BoothManagers set status="+status+" where open_id='"+userId+"' and booth_type=2"
            },
            ...
            success : function(res){
                if(res.confirm){
                    ...
                    wx.cloud.callFunction({
                        name : "executeSql",
                        data : {
							sql : "delete from BoothManagers where open_id='"+userId+"' and booth_type=2"
                        },
                        success : function(res){
                            wx.hideLoading()
                            wx.showToast({
    ...
    onShow() {
        ...
        wx.cloud.callFunction({
            name : "executeSql",
            data : {
				sql : "select a.status, b.* from BoothManagers a inner join Users b on a.open_id=b.open_ID where booth_type=2"
            },
            success : function(res){
				wx.hideLoading()
                that.setData({
            ...
            fail : function(res){
				wx.hideLoading()
                console.log("EditDunkTankMgr onLoad fail")
                // wx.showToast({
                //     title: "Load timeout",
                //     icon : "error"
                // })
